...
class mllibRecommendation (spark: SparkSession, dataRec: Dataset[Row], dataGames: DataFrame, metadata: DataFrame) {
  ...
  def recommend(targetUser: Int): Unit = {
    ...
    val (aggregateData, merged) = timeUtils.time(spark, preprocessData(), "Preprocessing Data", "MlLib")
    /*
    ...
    val tfidfValues = timeUtils.time(spark, calculateTFIDF(aggregateData), "Calculating TF-IDF", "MlLib")
    /*
    ...
    val topUsersSimilarity = timeUtils.time(spark, computeCosineSimilarity(tfidfValues, targetUser), "Getting Similar Users", "MlLib")
    /*
    Elapsed time for Getting Similar Users:	8360ms (8360099334ns)
    */
    // Time the recommendation generation
    println("Calculate final recommendation...")
    timeUtils.time(spark, getFinalRecommendations(merged, topUsersSimilarity, targetUser), "Generating Recommendations", "MlLib")
    /*
