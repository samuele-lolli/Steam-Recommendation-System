...
class sqlRecommendation(spark: SparkSession, dataRec: Dataset[Row], dataGames: DataFrame, metadata: DataFrame) {
  ...
  val targetVectorDF = tfidfDF.filter(col("user_id") === targetUser)
    .select("word", "tf_idf")
    .withColumnRenamed("tf_idf", "target_tfidf")

  val dotProductDF = tfidfDF
    .join(targetVectorDF, Seq("word"))
    .filter(col("user_id") =!= targetUser)
    .withColumn("dot_product", col("tf_idf") * col("target_tfidf"))
    .groupBy("user_id")
    .agg(sum("dot_product").alias("numerator"))

  val userNormsDF = tfidfDF
    .withColumn("squared_tfidf", col("tf_idf") * col("tf_idf"))
    .groupBy("user_id") // Group by user
    .agg(sqrt(sum("squared_tfidf")).alias("user_norm"))

  val targetNorm = targetVectorDF
    .withColumn("squared_target_tfidf", col("target_tfidf") * col("target_tfidf"))
    .agg(sqrt(sum("squared_target_tfidf")).alias("target_norm"))
    .as[Double]
    .head()

  val similarityDF = dotProductDF
    .join(userNormsDF, "user_id")
    .withColumn("cosine_similarity", col("numerator") / (col("user_norm") * lit(targetNorm)))
