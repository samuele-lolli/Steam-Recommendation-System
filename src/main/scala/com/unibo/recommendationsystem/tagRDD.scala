...
object tagRDD {
  def main(args: Array[String]): Unit = {
    ...
    val spark = builder
      ...
      .config("spark.executor.instances", "2") // Static number of executors for worker nodes
      .config("spark.dynamicAllocation.enabled", "true")
      .config("spark.dynamicAllocation.minExecutors", "2")  // Ensure stable minimum executors
      .config("spark.dynamicAllocation.maxExecutors", "3")  // Allow use of preemptible worker
      .config("spark.dynamicAllocation.executorIdleTimeout", "60s")
      .config("spark.speculation", "true") // Speculative execution to handle slow tasks
      .config("spark.speculation.interval", "100ms")
      .config("spark.speculation.multiplier", "1.5")
      .config("spark.sql.shuffle.partitions", "64")  // Adjust based on data size
      .config("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
      .config("spark.rdd.compress", "true")
      .config("spark.shuffle.file.buffer", "1m")  // Shuffle optimizations
      .config("spark.memory.fraction", "0.6")     // Allocate more memory for execution
